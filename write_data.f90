    ...
    disp = intsize + dbsize + (intsize+dbsize*N*N*N*numv)*disp

    call MPI_TYPE_CONTIGUOUS(1,MPI_INTEGER,tmptype,ierror)
    lb=0; extent=intsize+dbsize*N*N*N*numv; etype=MPI_INTEGER
    ...
    call MPI_TYPE_CONTIGUOUS(N*N*N*numv,MPI_DOUBLE_PRECISION,tmptype,ierror)
    lb=0; extent=intsize+dbsize*N*N*N*numv; etype=MPI_DOUBLE_PRECISION
    call MPI_TYPE_CREATE_RESIZED(tmptype,lb,extent,filetype,ierror)
    call MPI_TYPE_COMMIT(filetype,ierror)

    call MPI_FILE_SET_VIEW(fh,disp,etype,filetype,"native",MPI_INFO_NULL,ierror)
    call MPI_FILE_WRITE_ALL(fh,Q,NCELL*N*N*N*numv,MPI_DOUBLE_PRECISION,status,ierror)
